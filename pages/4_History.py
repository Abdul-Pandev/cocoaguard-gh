...
st.set_page_config(page_title='FarmEye · History', page_icon='🕘', layout='wide', initial_sidebar_state='expanded')
inject_css()
...
<div class="hero" style="padding:36px 40px;margin-bottom:28px;">
  ...
  <div class="hero-body">Every diagnosis run in this session is saved here. Track patterns across your farm and export records to share with your extension officer.</div>
</div>
...
if not history:
    ...
    with col_f1:
        filter_result = st.selectbox("Filter", ["All","CSSVD","Healthy"], label_visibility="collapsed")
    with col_f2:
    ...
    st.download_button("⬇ Download as CSV", data=csv, file_name="farmeye_history.csv", mime="text/csv")
    st.markdown('<br>', unsafe_allow_html=True)
