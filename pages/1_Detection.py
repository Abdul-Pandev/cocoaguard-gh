...
with st.sidebar:
    ...
    st.markdown('<div style="text-align:center;font-size:0.68rem;color:#2D6B38;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:16px;">Sankofa Intelligence</div>', unsafe_allow_html=True)

    st.markdown("""
    <div style="background:#F0FAF2;border:1px solid rgba(30,140,53,0.2);
                border-radius:10px;padding:12px 14px;margin-bottom:16px;">
      <div style="font-size:0.75rem;color:#1E8C35;font-weight:700;letter-spacing:0.5px;margin-bottom:6px;">
        🎙 Audio Language
      </div>
      <div style="font-size:0.71rem;color:#2D6B38;margin-bottom:8px;">
    ...
    st.markdown(f'<div style="font-size:0.78rem;color:#1E8C35;margin-bottom:4px;">✓ <b>{language}</b> selected</div>', unsafe_allow_html=True)
...
with col_upload:
    ...
    if uploaded_file:
        ...
        st.markdown("""
        <div style="border:2px dashed rgba(30,140,53,0.25);border-radius:12px;padding:48px 24px;
                    text-align:center;color:#2D6B38;background:#F7FCF8;">
...
with col_result:
    ...
    if not uploaded_file:
        st.markdown("""
        <div style="background:#F7FCF8;border:1px solid rgba(30,140,53,0.15);
                    border-radius:12px;padding:48px 24px;text-align:center;color:#2D6B38;">
          <div style="font-size:2.5rem;margin-bottom:12px;">🧠</div>
            ...
            try:
                ...
                if not is_cocoa:
                    ...
                    <div class="alert-cssvd">
                      ...
                      <p style="color:#7A1A1A;font-size:0.9rem;margin:8px 0 0;">
                    ...
                    if os.path.exists(audio_path):
                        st.markdown(f'<div style="color:#2D6B38;font-size:0.8rem;margin-top:16px;">🎙 Playing audio in <b style="color:#1E8C35;">{language}</b></div>', unsafe_allow_html=True)
                    ...
                    <div class="{alert_class}">
                      ...
                      <div style="font-size:2rem;font-family:'Syne',sans-serif;font-weight:800;margin:4px 0;color:#0A1A0C;">
                        {conf}% <span style="font-size:0.9rem;font-weight:400;color:#2D6B38;">confidence</span>
                      </div>
                      <p style="font-size:0.88rem;margin:8px 0 0;color:#0A1A0C;">{rec['summary']}</p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    urgency_color = "#C0392B" if rec['urgency'] == 'high' else "#1E8C35"
                    for action in rec['actions']:
                        st.markdown(
                            f'<div style="display:flex;gap:10px;margin-bottom:8px;">'
                            f'<span style="color:{urgency_color};">▸</span>'
                            f'<span style="color:#0A1A0C;font-size:0.9rem;">{action}</span></div>',
                            unsafe_allow_html=True
                        )
                    ...
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                      ...
                      <div>
                        <div style="color:#0A1A0C;font-size:0.85rem;font-weight:600;">Audio Diagnosis</div>
                        <div style="color:#2D6B38;font-size:0.75rem;">Playing in <b style="color:#1E8C35;">{language}</b></div>
                      </div>
                    </div>
                    """, unsafe_allow_html=True)
...
divider()
st.markdown('<p style="color:#2D6B38;font-size:0.72rem;text-align:center;">⚠ Always consult a qualified agricultural officer for final decisions · Sankofa Intelligence · 2026</p>', unsafe_allow_html=True)
