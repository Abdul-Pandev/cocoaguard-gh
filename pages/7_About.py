...
<div class="hero" style="padding: 36px 40px; margin-bottom: 28px;">
  ...
  <div class="hero-title" style="font-size: 2rem;">ℹ About FarmEye</div>
...
<div class="feature-card" style="margin-bottom: 16px;">
  <p style="color:#C8DFC9; font-size:0.98rem; line-height:1.85; margin:0;">
    FarmEye exists to <b style="color:#E8F5E9;">democratise access to agricultural intelligence</b>. 
    We believe that every cocoa farmer in Ghana — regardless of education level, language, or 
    proximity to specialist services — deserves the same early warning capabilities as the 
    best-resourced research institutions.
  </p>
  <p style="color:#C8DFC9; font-size:0.98rem; line-height:1.85; margin: 16px 0 0;">
    CSSVD does not discriminate. Neither should early detection.
  </p>
</div>
""", unsafe_allow_html=True)
...
with c1:
    ...
    <div class="feature-card">
      <p style="color:#C8DFC9; font-size:0.92rem; line-height:1.85;">
        <i>"Se wo were fi na wosankofa a yenkyi"</i><br>
        <span style="color:#7AAB80; font-size:0.82rem;">(Twi) — "It is not wrong to go back for what you forgot."</span>
      </p>
      <p style="color:#C8DFC9; font-size:0.92rem; line-height:1.85; margin-top: 14px;">
        The Sankofa bird looks backward while moving forward — a symbol of learning from 
        the past to build a better future. Our AI is trained on historical disease data 
        to protect future harvests: Sankofa in practice.
      </p>
    </div>
    """, unsafe_allow_html=True)
with c2:
    ...
    <div class="feature-card" style="text-align:center; padding: 32px 24px;">
      ...
      <div style="font-family:'Syne',sans-serif; font-weight:700; color:#C9A84C; font-size:1.1rem;">Sankofa</div>
      <div style="color:#7AAB80; font-size:0.78rem; margin-top:4px; line-height:1.5;">Akan Adinkra symbol<br>of wisdom & learning</div>
    </div>
    """, unsafe_allow_html=True)
...
stack = [
    [("🧠 TensorFlow", "Deep learning model training & inference"),
     ("⚡ EfficientNetB3", "Pretrained CNN backbone (ImageNet)"),
     ("🔁 JAX", "Additional ML computation support")],
    [("🌐 Streamlit", "Mobile-first web application framework"),
     ("📊 Plotly", "Interactive analytics visualisations"),
     ("🖼 Pillow", "Image preprocessing & handling")],
    [("🐍 Python 3.11", "Core application language"),
     ("🎙 gTTS / Audio", "Multilingual text-to-speech output"),
...
for col, items in zip([t1, t2, t3], stack):
    with col:
        for name, desc in items:
            st.markdown(f"""
            <div style="background: rgba(15,34,18,0.6); border: 1px solid rgba(61,204,82,0.12); border-radius: 8px; padding: 12px 14px; margin-bottom: 10px;">
              <div style="font-weight:600; font-size:0.9rem; color:#E8F5E9; margin-bottom:3px;">{name}</div>
              <div style="color:#7AAB80; font-size:0.78rem;">{desc}</div>
            </div>
            """, unsafe_allow_html=True)
...
<div style="
  ...
  line-height: 1.8;
">
  FarmEye is designed to <b>assist</b> farmers and extension officers, not replace professional agricultural advice.
...
st.markdown("""
<div class="feature-card">
  <div style="color:#C8DFC9; font-size:0.9rem; line-height:1.85;">
    We thank the <b style="color:#E8F5E9;">Ghana Cocoa Board (COCOBOD)</b>, the 
    <b style="color:#E8F5E9;">Cocoa Research Institute of Ghana (CRIG)</b>, and the 
    dedicated extension officers and farming communities who made this dataset possible.
    <br><br>
    FarmEye is a product of <b style="color:#3DCC52;">Sankofa Intelligence</b> — Ghana · 2026.
  </div>
</div>
""", unsafe_allow_html=True)

divider()
st.markdown('<p style="color:#3A5C3E; font-size:0.72rem; text-align:center;">Sankofa Intelligence · Ghana · 2026 · Open Source</p>', unsafe_allow_html=True)
